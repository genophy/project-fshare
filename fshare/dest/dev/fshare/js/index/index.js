...
/***/ (function(module, exports, __webpack_require__) {
	...
	$(function() {
		 ...
		 * @param pathNamesArr
		 */
		const fillDirCrumbs = function(pathNames) {
			globalPathNames    = pathNames;
			const pathNamesArr = pathNames.split('/');
			pathNamesArr.forEach(function(item) {
				if (item) {
					const $dirChild = $('<div class="dir__child">');
					$dirChild.text(item);
		 ...
		 * @param list
		 */
		const fillFileList = function(pathNames, list) {
			changeGlobalChkbox(0);
			if (list && 0 < list.length) {
				list.forEach(function(item) {
					const $temp = $('<div class="item">');
					const $chk  = $('<div class="item--check chk-box">');
					const $div  = $('<div class="item--name">');
		 ...
		 * @param pathNames
		 */
		const cdDir = function(pathNames) {
			dialogUpload.close();
			$('iframe').remove();
			// set path-children
		...
		$('.btn-download').on('click', function(e) {
			...
			for (let i = 0; i < allChk.length; i++) {
				...
				// downloadFiles('/rest/file/download?fileName=' + globalPathNames + ('/' + $(allChk[i]).next().text()).replace('/', '---'));
				// console.log( ('/' + $(allChk[i]).next().text()).replace('/', '---'));
				// window.open('/rest/file/download?fileName=' + globalPathNames + ('/' + $(allChk[i]).next().text()).replace('/', '---'), '_blank');
			}
			downloadAll(files);

		});

		const initIpAddress = function() {
			$.get('/rest/address/getlocal', function(data) {
				$('.ip-address').val('http://' + data + ':8000');
			});
		};

		const initFileRootList = function() {
			cdDir('.');
		};
